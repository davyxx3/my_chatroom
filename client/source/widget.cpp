...
Widget::Widget(QWidget *parent)
    ...
    connect(readClient, &ReadClient::onlineDisplayAble, this, [=]() {
        ...
        QVariantList onlineList = QJsonDocument::fromJson(
                QString::fromStdString(readClient->getTextContent()).toUtf8()).toVariant().toList();
        for (auto &user: onlineList) {
            QVariantMap map = user.toMap();
            QString element =
                    map["account"].toString() + "\t" + map["username"].toString() + "\t" + map["loginTime"].toString();
